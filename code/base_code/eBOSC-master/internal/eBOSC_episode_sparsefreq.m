...
%     disp('Creating sparse detected matrix ...')
    ...
    %tmp_B    = TFR.*detected;
    detected = zeros(size(tmp_B)); %%modifies from size(detected) so that there could be additions for a thicker line
    ...
    for f = range(1,1)+1:size(tmp_B,1)-range(1,2) % loop across frequencies. note that indexing respects the appended segments
        ...
        detected(f-1,:) = detected(f-1,:) + (mean(tmp_det,1) == 1); %%added by me, Eugenio
        detected(f,:) = detected(f,:) + (mean(tmp_det,1) == 1); %%%% modified to addition over zeros, to make thicker line.
        detected(f+1,:) = detected(f+1,:) + (mean(tmp_det,1) == 1); %%%added by me, Eugenio

    end; clear f;
    detected = detected>0; %%%added by me, Eugenio
    % remove padded zeros
