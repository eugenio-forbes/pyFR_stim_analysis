...
sesscodes = unique(sesscodes([hippoStruct(:).hasRS]&[hippoStruct(:).hasNS]));

title = 'TimexCon_RSNS_0-2_PCCB_slope'
formula = 'pow ~ time*condition + (1+condition|sub:elec)';
type = 'retrieval2_median_band';
norm = 'nostim';
variable = 'normPow';
conditions = {'RS','NS'};
times = [0,2];
...
tableDir = fullfile(rootDir,'Tables_WM_retrieval2_median_band_add',norm,variable);
saveDir =  fullfile(rootDir,'LMEM_time_new_add',type,norm,variable,title);
...
for freq = 1:6
    ...
    t.condition = contains(t.condition,'RS');
    t.time = t.time == 2;
