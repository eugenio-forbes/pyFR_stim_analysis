...
%can be exlcluded from the montage (e.g., UP044_1 had scalp eeg contacts as
%well).
...
fprintf(fout1,'samplerate %.2f\ndataformat ''%s''\ngain %g\n',SampleRate,outdataformat,Cal(1));
fclose(fout1);
...
for c = 1:size(Label,1)
  ...
  if ~isempty(regexp(thisLabel,'EEG ')) 
    ...
    thisLabel = regexprep(thisLabel,'EEG ','');
      ...
      if ~isempty(regexp(thisLabel,'\$')) 
    fprintf('\n\tWARNING: removing ''$'' in %s',['''' thisLabel '''']);
    thisLabel = regexprep(thisLabel,'\$','BAD');
  end
    if ~isempty(regexp(thisLabel,'POL ')) 
    fprintf('\n\tWARNING: removing ''POL '' in %s',['''' thisLabel '''']);
    thisLabel = regexprep(thisLabel,'POL ','');
  end
  ...
  if any(~cellfun(@isempty,regexp(thisLabel,badLabels)))
