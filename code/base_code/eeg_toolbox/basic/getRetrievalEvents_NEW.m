...
for s=1:length(unSess)
  ...
  for l=1:length(unListThisSess)
    ...
    if size(staEvThisSessList,2) > 1 % modified sarah seger, fixed issue that arises when staEvThisSessList is longer than 1
       staEvThisSessList = staEvThisSessList(1); 
    end
    ...
    % modified: Sarah Seger 03/06/2018
    % accounts for issue with testElec counting to infinity when
    % staEvThisSessList contains more than one eegfile
    
    if isempty(thisEEGfile) | ~strcmp(thisEEGfile,staEvThisSessList.eegfile);
       % only use first EEG file
        if size(staEvThisSessList,2) > 1
            staEvThisSessList_temp = staEvThisSessList;
            staEvThisSessList = staEvThisSessList(1);
        end
        %
	...
	try
      ...
      % set staEvThisSessList back to original
      if exist('staEvThisSessList_temp', 'var')
          staEvThisSessList =  staEvThisSessList_temp;
          clear vars staEvThisSessList_temp
      end
      % 
