function bpTalStruct = getMonopolarTalStructureFromOldStuff(subj)        
%
%
%
%
...
for k=1:length(inMom_elecs)
  ...
  if hasIndiv
   ...
   thisTag_idx = elecNum==thisElec(1);
   if ~isempty(thisTag_idx)
     thisTag   = elecNam{thisTag_idx};
   else
     thisTag   = 'XXX';
   end
  ...
  bpTalStruct(k).grpName = elecNam_stripped{thisTag_idx};
  bpTalStruct(k).eNames   =  num2str(thisElec);
