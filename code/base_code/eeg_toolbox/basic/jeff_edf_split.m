function jeff_edf_split(eegfile,subject,outputdir)
% jeff_edf_split - Splits a Jefferson Hospital EDF datafile into separate channels into
...
dataFilePos = ftell(fid);
for c = chan
    fprintf('%d ',c);    
    raw = int16(zeros(1,datLen));
    for t = 1:NRec
      ...
      if length(dat)>0
        if length(dat)<NS*SPR
          ...
          raw((t-1)*SPR+1:t*SPR) = int16(dat2(:,c)');
        end
      end
    end
    ...
    fwrite(fchan,raw,outdataformat);
    fclose(fchan);
        
    fclose(fid);
    [fid,message] = fopen(eegfile,'rb','ieee-le');
    fseek(fid,dataFilePos,-1);
end
fprintf('\n');
