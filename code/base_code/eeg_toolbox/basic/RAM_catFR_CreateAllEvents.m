function RAM_catFR_CreateAllEvents(subject,expDir,session,forceSession, startingElec)
%
% FUNCTION:
...
if ~exist(evFile,'file')
  events=RAM_catFR_CreateTASKEvents(subject,expDir,session,forceSession, startingElec);
  if isempty(events)
    return
  end
  save(evFile,'events');
  cd(thisSessDir); unix('chgrp RAM_eeg events.mat');
  clear events
  fprintf('DONE.\n')
else
...
if ~exist(mevFile,'file')
  [events MATHcfg]=RAM_catFR_CreateMATHEvents(subject,expDir,session,forceSession);
