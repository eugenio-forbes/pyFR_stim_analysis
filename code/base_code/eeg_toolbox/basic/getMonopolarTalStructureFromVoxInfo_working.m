function [talStruct, hasMom] = getMonopolarTalStructureFromVoxInfo_working(subj,doTalDaemon,source_subj)
%
% FUNCTION:
...
talDir_source = fullfile(subjDir_source,'tal/talFiles_old');
...
for k=1:length(inMom_elecs)
  ...
  talStruct(k).tagName  = elecNam{k,1};
  talStruct(k).grpName = elecNam_stripped{k};
