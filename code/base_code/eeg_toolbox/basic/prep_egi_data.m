function prep_egi_data(subject,session,eventfiles,badchan,ms_field,captype)
...
for i=1:length(rawfile)
  ...
  basename = egi_split(fullfile(eegdir,rawfile(i).name),subject,norerefdir);
...
samplerate = GetRateAndFormat(fullfile(norerefdir,'params.txt'));
beh_file = {'eeg.eeglog.up'};

% align data
runAlign(samplerate, beh_file, eeg_file, chan_file, eventfiles, ms_field, 0, 1);
...
reref(fileroots,[1 129],rerefdir,{1:129,setdiff(channels,badchan)});
