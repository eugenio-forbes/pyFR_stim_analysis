function tal_coords = vox2tal_working(subj, CT_coords, outfile, reg_file)
...
% Create variable reg_avg_file if it is not passed in in the default
% location
if ~exist('reg_file','var')
    reg_file = fullfile('/data/eeg',subj,'tal/images/combined/reg_avg_flipped.lta');
end
...
[~,results] = system(sprintf('mri_info --vox2ras-tkr %s',...
    fullfile('/data/eeg',subj,'/tal/images/combined',[subj '_CT_combined_flipped.img'])));
