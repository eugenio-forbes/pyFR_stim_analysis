function convertMotherToVoxCoords(subj,source_subj, doPlot, force_redo)
...
if ~exist(voxActualFile,'file') || ~exist(tal_file,'file') || force_redo 
    ...
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    %  register
    fprintf('Registering the VOX file to talairach space\n')
    

    % run the vox coords through the registration file
    origDir = pwd;
    if ~exist(mni_file,'file') || force_redo
        cd(talDir)
        vox2raw(vox_coords,imageroot,ct_combined,ct2mr,mr_brain,mr2standard,false);
        fprintf('   I just made %s\n',mni_file)
    else
        fprintf('   %s already exists\n',mni_file)
    end
        
    % make the talairach file
    if (~exist(tal_file,'file') || force_redo) && exist('RAW_coords.txt.mni','file')
        convertFile_mni2tal('RAW_coords.txt.mni');
        fprintf('   I just made %s\n',tal_file)
    else
        fprintf('   %s already exists\n',tal_file)
    end
...
if force_redo
    system(['rm ' (fullfile(dataDir,subj,'tal/bpPairs.mat'))]);
end
...
reg_avg_file = fullfile(dataDir, source_subj, 'tal/images/combined/reg_avg.lta');
cd(talDir)
if (~exist(avgSurf_file,'file') || force_redo) && ... 
    ...
    isDepth = getIsDepth_local(fullfile(source_talDir,voxMotherName), ...
    ...
    energyMinimization(subj, source_subj, avgSurf_file, [], isDepth, true, false, avgSurfEsnap_file);
    fprintf('    I just made %s\n',avgSurfEsnap_file);
else
    if exist(avgSurf_file,'file') && ~force_redo
        fprintf('    %s already exists\n', avgSurf_file);
...
reg_file = fullfile(dataDir, source_subj, 'tal/images/combined/reg.lta');
cd(talDir)
if (~exist(indivSurf_file,'file') || force_redo)  && exist(reg_file,'file')
    ...
    % Now snap this to surface using the snap method
    snapElectrodesToSurface(source_subj, indivSurf_file, isDepth, false, indivSurfSnap_file, [], false, [], [], [], 20);
    fprintf('     I just made %s\n',indivSurfSnap_file);
    % and then again using the spring method


    energyMinimization(subj, source_subj, indivSurf_file, [], isDepth, false, false, indivSurfEsnap_file);
    fprintf('    I just made %s\n',indivSurfEsnap_file);
else
...
talStruct = getMonopolarTalStructureFromVoxInfo(subj, [], source_subj);
cd(talDir)
if ~hasMom %this should only happen if its an old subject, then we will use the old bp_mat method
    bpTalStruct = getBipolarTalStructureFromOldStuff(subj);
    talStruct = getMonopolarTalStructureFromOldStuff(subj);
end
cd(talDir)
save(bpTalFile,'bpTalStruct');
save(talFile, 'talStruct');
%keyboard
