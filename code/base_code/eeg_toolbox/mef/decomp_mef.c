//
//mex decomp_mef.c RED_decode.c RED_encode.c endian_functions.c mef_lib.c AES_encryption.c crc_32.c
...
void decomp_mef(char *f_name, unsigned long long int start_idx, unsigned long long int end_idx, int *decomp_data, char *password)
{
    ...
    unsigned char       cpu_endianness();
	int			*dcdp, *temp_data_buf;
	unsigned int		n_read, comp_data_len, bytes_decoded, tot_samples;
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
	...
	if (mxIsChar(prhs[3]) != 1) { // Check to make sure the fourth input argument is a string 
		mexErrMsgTxt("[decomp_mef] Password must be a stringx => exiting");
		return;
	}	
