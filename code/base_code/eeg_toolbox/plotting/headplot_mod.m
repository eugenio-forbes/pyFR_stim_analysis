...
function [] = headplot_mod(values, arg1, varargin)
    ...
    for i = 1:size(newElect,1)
      ...
      if newElect(i,:) ~= [0 0 0]  % plot radial lines to electrode sites
        ...
        elseif g.labels == 2   % plot electrode names
          ...
          % PBS - Only plot good elec
          goodelec = setdiff(1:size(newElect,1),g.elec_noplot);
          line(newElect(goodelec,1),newElect(goodelec,2),newElect(goodelec,3),'marker',...
