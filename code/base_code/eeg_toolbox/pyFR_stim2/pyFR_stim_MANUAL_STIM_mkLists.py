...
def makeStimForms(exp, trialList_1,trialList_2):
    """ 
    ...
    preamble.append("\\usepackage{longtable}\n")
    ...
    preamble.append("\\item Patient Name OR Subject Code: \_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\\\\\n")
    preamble.append("\\item Date: \_\_\_\_\_\_/\_\_\_\_\_\_/\_\_\_\_\_\_\\\\\n")
    preamble.append("\\end{enumerate}\n")
    stimForm.writelines(preamble)
    ...
    # outer loop: make tables of trialsPerPage columns till we've done all trials.
    while currentTrialRangeStart<config.numTrials:
	
	# declare 2 columns (word + checkbox) for each word in a trial
	table_format = ''
	numColsToMake = min(5, config.numTrials-currentTrialRangeStart)
	table_format = '|cccc'*numColsToMake
	stimForm.write("\\begin{longtable}{c%s}\n" % table_format)

	# make column headers:
	headerline = 'trial & '
	for i in range(numColsToMake):
	    headerline += '%d & V & R & S & ' % (currentTrialRangeStart+i+1)
	
	headerline = headerline[:-3]
	headerline += '\\\\ \\hline \\hline\n'
	stimForm.write(headerline)
	...
	for j in range(wpt):
	    ...
	    for i in range(numColsToMake):
                wordStim  = 0
		#thisline += '%s & $\\square$ & $\\square$ & %d & '%(trialList_1[i+currentTrialRangeStart][j]['name'], wordStim)
                thisline += '%s & $\\square$ & $\\square$ & %s & '%(trialList_1[i+currentTrialRangeStart][j],
                                                                    trialList_2[i+currentTrialRangeStart][j])
	    thisline  = thisline[:-3] 	# drop the last ampersand
	    thisline += '\\\\\n'
	    stimForm.write(thisline)

	stimRow = 'Stim & '
	for i in range(numColsToMake):
	    if stimOrNot[i+currentTrialRangeStart]:
		stimRow += 'Y & & & & '
	    else:
		stimRow += 'N & & & &'
	stimRow = stimRow[:-6]
	stimRow += '\\\\\n'
	stimForm.write(stimRow)

	blanksRow(stimForm, "motor", numColsToMake, 6)
	blanksRow(stimForm, "mAmps", numColsToMake, 6)
	blanksRow(stimForm, "Contact$_1$", numColsToMake, 6)
	blanksRow(stimForm, "Contact$_2$", numColsToMake, 6)

	stimForm.write("\\end{longtable}\n")
	stimForm.write('\\pagebreak')
	currentTrialRangeStart += numColsToMake

    stimForm.write("\\end{document}\n")
    stimForm.close()

    # compile the LaTeX
    print "Compiling latex form..."
    os.chdir(exp.session.fullPath())
    command = 'pdflatex %s' % formName+'.tex'
    os.system(command)
...
def prepare(exp, config):
    ...
    for sessionNum in xrange(config.numSessions):
        ...
        # write the 
        makeStimForms(exp, trialList_loImag, trialList_hiImag)
