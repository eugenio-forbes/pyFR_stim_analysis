function analyzeTJ055pyfrStim(subj)
...
for k=1:length(elStimNames)
  ...
  % plot the accuracies
  figure(1)
  barVect_1 = [numCorrs_b numCorrs_e numCorrs_r numCorrs_m];
  barVect_2 = [numWords_b numWords_e numWords_r numWords_m];
  ...
  set(gca,'XTickLabel',{'bas','enc','ret','del ret'})
  ylim([.7 1])
  formatPic(gca)
  ...
  % anova on the time  
  D = [frt_b; frt_e; frt_r; frt_m];
  G = [repmat({'bas'},size(frt_b)); repmat({'enc'},size(frt_e));...
       repmat({'ret'},size(frt_r)); repmat({'del ret'},size(frt_m))];
