function [varargout] = phasepowMP_EF(chan,events,DurationMS,OffsetMS,BufferMS,varargin)
%GETPHASEPOW - Calculate wavelet phase and power for a set of events with a
% POLY-POLAR reference scheme
%
...
% load the eeg for reference electrodes
refEEGs = [];
for idx = 2:size(chan,2)
    ph = gete_ms(chan(idx),events,DurationMS+(2*BufferMS),OffsetMS-BufferMS,0,filtfreq,filttype,filtorder,resampledrate);
    refEEGs = cat(3, refEEGs, ph);
end
eeg = gete_ms(chan(1),events,DurationMS+(2*BufferMS),OffsetMS-BufferMS,0,filtfreq,filttype,filtorder,resampledrate)-nanmean(refEEGs,3);
...
if ~isempty(kthresh)
    ...
    if ~all(isnan(k))
        goodInd = k<=kthresh;
        events_badk_ind = find(k>kthresh);
        events_goodk = events(k<=kthresh);
        eeg_goodInd = eeg(goodInd,:);
    else
        goodInd = 1:length(events);
        events_badk_ind = [];
        events_goodk = events;
        eeg_goodInd = eeg;
    end
    minusMedian = bsxfun(@minus, eeg_goodInd, median(eeg_goodInd));
    dc_norm = sqrt(sum(minusMedian.^2,2));
    arti_ind = dc_norm >= prctile(dc_norm, 95);
    events_arti_ind = find(ismember([events(:).eegoffset],[events_goodk(arti_ind).eegoffset]));
    events_bka = [events_badk_ind events_arti_ind];
    goodInd = find(~ismember([1:length(events)],events_bka));
