...
subject_behavioral_table = check_bad_subjects(root_directory,subject_behavioral_table);
...
frequency_bands = {'slow_theta'};%{'slow_theta';'fast_theta'};
anatomical_regions = {'anterior';'posterior'};
hemispheres = {'ipsilateral';'contralateral'}; %{'left';'right';'ipsilateral';'contralateral'};
stimulation_hemispheres = {'ipsilateral';'contralateral'}; %{'left';'right';'ipsilateral';'contralateral'};
...
%             
% %             power_table = check_bad_subjects(root_directory,power_table);
%             
%             is_anterior = logical(power_table.anterior);
%             
...
%             end
%             
%             is_left = logical(power_table.left);
%             stimulation_left = logical(power_table.stimulation_left);
...
%                 same_group = logical(power_table.stimulation_lateral);
%             else
%                 same_group = ~logical(power_table.stimulation_lateral);
%             end
...
%             stim_site_table = check_bad_subjects(root_directory,stim_site_table);
...
%             phase_table = check_bad_subjects(root_directory,phase_table);
...
switch effect_type
    ...
    case 'phase'
        ...
        plot_y_axis = false;
        plot_x_axis = true;
        max_x = max(independent_variable);
        x_limits = [0,0.5];
        x_ticks = 0:0.1:0.5;
        y_limits = [-3,3];
        y_ticks = -3:1:3;
